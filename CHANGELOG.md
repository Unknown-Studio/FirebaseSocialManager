...
- **Chat & Guild Timestamp:** Fixed an issue where `msg.Timestamp` returned `null` for newly sent messages due to Firestore's local echo. Now using `ServerTimestampBehavior.Estimate` to provide an estimated timestamp for pending local writes.
